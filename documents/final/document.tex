...
Device                                                          Controller
  ...
  | <---- ECDH_pub_C, Sign(Device_ID | ECDH_pub_D | ECDH_pub_C) ---- |
  |   [Both derive K_dc = HKDF(ECDH_secret, transcript)]             |
  | <--- {Device_ID, counter_init=0}_{K_dc} ------------------------ |
\end{verbatim}

The controller signs the full handshake transcript (Device\_ID, ECDH\_pub\_D, ECDH\_pub\_C). This binds authentication to this specific pairing session and prevents replay or substitution of authenticated key shares.
...
The controller obtains randomness from the operating system's cryptographically secure pseudorandom number generator (CSPRNG), such as \texttt{getrandom()} or \texttt{/dev/urandom} on Linux systems. These interfaces are seeded from multiple system entropy sources and are suitable for generating cryptographic keys. The smart devices need to pull from hardware TRNGs if they have them, and they absolutely cannot use factory-cloned deterministic keys.
